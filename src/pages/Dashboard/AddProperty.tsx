...
import { MapPin, Camera, Upload, FileText, AlertCircle, CheckCircle, ArrowRight, ArrowLeft } from 'lucide-react';
...
export function AddProperty() {
  ...
  const buildingMaterials = [
    'Concrete/Block', 'Brick', 'Wood Frame', 'Steel Frame', 'Mixed Materials'
  ];

  const occupancyTypes = [
    'Owner Occupied', 'Tenant Occupied', 'Vacant', 'Under Construction'
  ];
  ...
  const validateStep = (step: number) => {
    ...
    if (step === 1) {
      ...
      if (!formData.lga.trim()) newErrors.lga = 'Local Government Area is required';
      if (!formData.propertyType) newErrors.propertyType = 'Property type is required';
      if (!formData.yearBuilt) newErrors.yearBuilt = 'Year built is required';
      if (!formData.materials) newErrors.materials = 'Building materials are required';
      if (!formData.occupancy) newErrors.occupancy = 'Occupancy type is required';
    }

    if (step === 2) {
      if (!formData.mapConfirmed) newErrors.map = 'Please confirm your property location on the map';
    }

    if (step === 3) {
      if (formData.photos.length === 0) newErrors.photos = 'At least one property photo is required';
      if (formData.documents.length === 0) newErrors.documents = 'Property documents are required';
    }
  ...
  const handleSubmit = async () => {
    try {
      ...
      addToast({
        ...
        message: 'Your application is now being processed. Track your progress from the dashboard.'
      });
  ...
  const steps = [
    ...
    { number: 2, title: 'Location & Map', description: 'Confirm your property location' },
    { number: 3, title: 'Photos & Documents', description: 'Upload media and supporting documents' }
  ];
              ...
              <div className="space-y-6 animate-fade-in">
                <div>
                  ...
                  <p className="text-gray-600 dark:text-gray-400 mb-6">
                    Tell us about your property so we can provide accurate coverage options.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-1">
                    ...
                    {errors.propertyType && <p className="text-sm text-red-600 dark:text-red-400">{errors.propertyType}</p>}
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-1">
                    ...
                    {errors.materials && <p className="text-sm text-red-600 dark:text-red-400">{errors.materials}</p>}
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    {errors.occupancy && <p className="text-sm text-red-600 dark:text-red-400">{errors.occupancy}</p>}
                  </div>
                </div>
              </div>
            )}
            ...
            {currentStep === 2 && (
              <div className="space-y-6 animate-fade-in">
                <div>
                  ...
                  <p className="text-gray-600 dark:text-gray-400 mb-6">
                    Please confirm your property's exact location on the map. This helps us provide accurate coverage.
                  </p>
                </div>
                ...
                <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-8 text-center bg-gray-50 dark:bg-gray-800">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                    Map integration will show here. Click to drop a pin at your exact property location.
                  </p>
                  ...
                  {formData.mapConfirmed ? (
                    <div className="flex items-center justify-center space-x-2 text-green-600 dark:text-green-400">
                      ...
                      <span className="text-sm">({formData.latitude}, {formData.longitude})</span>
                    </div>
                  ) : (
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-blue-700 dark:text-blue-400 mt-1">
                        Precise location helps us assess risk factors like flood zones, fire stations proximity, 
                        and local crime rates to provide you with the most accurate premium.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {currentStep === 3 && (
              <div className="space-y-6 animate-fade-in">
                <div>
                  ...
                  <p className="text-gray-600 dark:text-gray-400 mb-6">
                    Upload fresh photos and supporting documents. Photos must be taken within 72 hours with location enabled.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center hover:border-blue-400 transition-colors cursor-pointer"
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      Take fresh photos with location enabled. Include exterior and interior shots.
                    </p>
                  </div>
                  ...
                  {errors.photos && <p className="text-sm text-red-600 dark:text-red-400 mt-2">{errors.photos}</p>}
                </div>
                ...
                <div>
                  ...
                  {errors.documents && <p className="text-sm text-red-600 dark:text-red-400 mt-2">{errors.documents}</p>}
                </div>
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-yellow-700 dark:text-yellow-400 mt-1">
                        All photos must be taken within the last 72 hours with location services enabled. 
                        This helps prevent fraud and ensures accurate property assessment.
                      </p>
                    </div>
                  </div>
                </div>
